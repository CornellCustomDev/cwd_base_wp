...
add_action( 'enqueue_block_editor_assets', 'cwd_enqueue_block_editor_styles' );

function cwd_enqueue_block_editor_styles() {
 ...
 */
add_filter( 'block_categories_all', 'cwd_block_categories', 10, 2 );

function cwd_block_categories( $block_categories, $block_editor_context ) {
	// General CWD blocks
 ...
 */
add_action( 'init', 'cwd_register_blocks' );

function cwd_register_blocks() {
	// General-use blocks
 ...
 * Events block query
 */
function cwd_events_block_query( $count, $taxonomy_terms ) {
	$query_args = array(
		'post_type' => 'events',
	...
	if ( $taxonomy_terms ) {
		$tax_query = _cwd_generate_tax_query( $taxonomy_terms );
 ...
 * News block query
 */
function cwd_news_block_query( $taxonomy_terms ) {
	$query_args = array(
		'post_type' => 'news',
		'posts_per_page' => 3,
		'orderby' => 'meta_value',
 ...
 */
function _cwd_generate_tax_query( $taxonomy_terms ) {
	$tax_query = array();
 ...
 */
add_filter( 'allowed_block_types_all', 'cwd_allowed_block_types', 10, 2 );

function cwd_allowed_block_types( $allowed_block_types, $block_editor_context ) {
