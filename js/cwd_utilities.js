   ...
   (function ($, root, undefined) { $(function () { 'use strict';
	   ...
	   $('.content-tabs').each(function() {
		   ...
		   $(this).before('<'+nav_tag+' aria-label="Choose a Tab" class="'+tab_classes+'"></'+nav_tag+'>').addClass('scripted').children('li').each(function(i){
			   ...
			   if (aria_mode) {
				   $(this).parent().prev(nav_tag).attr('role', 'tablist').append('<'+tab_tag+' role="tab" tabindex="-1" aria-selected="false" aria-controls="'+tab_id+'">'+ tab_number + tab_labelbefore + tab_title + tab_labelafter + '</'+tab_tag+'>');
				   var tab_bg = $(this).attr('data-bg-img');
				   $(this).attr('id',tab_id).attr('role','tabpanel').wrapInner('<div class="tab-inner"></div>').attr('aria-label', tab_title).addClass('aria-target').hide();
				   if (tab_bg !== undefined) {
					   $(this).children('.tab-inner').css('background-image', 'url(' + tab_bg + ')');
				   }
		   ...
		   $(this).prev(nav_tag).each(function() {
			   ...
			   $(this).children(tab_tag).first().addClass('active').attr('aria-selected','true').attr('tabindex', '0');
				   ...
				   $(this).children(tab_tag).each(function() {
					   ...
					   $(this).keydown(function(e) {
						   if (e.code == "ArrowLeft") { // left arrow key
							   e.preventDefault();
							   $(this).siblings().addBack().eq(prev_tab).trigger('click').focus().attr('tabindex', '0');
							   $(this).siblings().addBack().eq(prev_tab).siblings().attr('tabindex', '-1');
						   }
						   else if (e.code == "ArrowRight") { // right arrow key
							   e.preventDefault();
							   $(this).siblings().addBack().eq(next_tab).trigger('click').focus().attr('tabindex', '0');
							   $(this).siblings().addBack().eq(next_tab).siblings().attr('tabindex', '-1');
						   }
					   });
				   });
			   }
