...
/*
 * WP Saml Auth config (see https://wordpress.org/plugins/wp-saml-auth)
 *
 * By default, all options are configured for test environments
 *
 * To go live:
 * - Uncomment // Live 'idp' config and delete or comment out // Test 'idp' config
 * - Set permit_wp_login to false to prevent non-saml login on production site
*/
if ( ! function_exists ( 'cwd_simplesaml_auth' ) ) {

	function cwd_simplesaml_auth( $value, $option_name ) {
		$defaults = array(
			...
			'internal_config' => array(
				...
				'idp' => array(
					...
					'x509cert' => file_get_contents(ABSPATH . '/private/simplesaml/cert/cornell-idp-live.cer'),
				), */
				...
				'idp' => array(
					...
					'x509cert' => file_get_contents(ABSPATH . '/private/simplesaml/cert/cornell-idp-test.cer'),
				),
			),
			...
			// Set to false if users need to have existing local WP accounts
			'auto_provision' => true,
	...
	add_filter( 'wp_saml_auth_option', 'cwd_simplesaml_auth', 10, 2 );
}

if ( ! function_exists ( 'cwd_simplesaml_login_text' ) ) {
	function cwd_simplesaml_login_text( $strings ) {
	...
	add_filter( 'wp_saml_auth_login_strings', 'cwd_simplesaml_login_text' );
}