...
if ( ! class_exists( 'OG_Tags_Front' ) ) {
	...
	class OG_Tags_Front {
		...
		private function insert_post_tags() {
			...
			$image_id   = get_field('image_id'); // ACF image field
			$featured_image_id = get_post_thumbnail_id($post->ID); // Featured (Header) image
			...
			elseif (get_field('image_id')) {
				$ogimage = wp_get_attachment_image_src($image_id)[0]; 
			} 
			elseif ( has_post_thumbnail() ) {     
				$ogimage = wp_get_attachment_image_src($featured_image_id)[0]; 
			}
			elseif ( has_header_image() ) {     
				$ogimage = get_header_image(); // Main header image from Customizer
			}
			else {
				$ogimage = esc_attr( $options['ogtags_image_default'] ); // Default image from settings page
			}
