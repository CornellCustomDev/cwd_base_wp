...
// Breadcrumbs
if ( ! function_exists ( 'cwd_base_breadcrumbs' ) ) {
	
	function cwd_base_breadcrumbs() {
		
		$home_page_options = get_field('home_page_options', 'options');
		$show_or_hide = $home_page_options['remove_breadcrumbs'];
		$show_or_hide == 1 ? $show_or_hide = 0 : $show_or_hide = 1;
		...
		$show_on_home   = $show_or_hide; // 1 - show breadcrumbs on the blog page, 0 - don't show
		...
		global $post;
		$home_url       = home_url('/');
		$link           = '<li><a href="%1$s"><span class="limiter">%2$s</span></a></li>';
		...
		if ( is_front_page() ) {
			...
			if ( is_category() ) {
				...
				if ( get_post_type() != 'post' ) {
					...
					if ( $position > 1 ) echo $sep;
					echo sprintf( $link, $home_url.$post_type->name, $post_type->labels->name, $position );
